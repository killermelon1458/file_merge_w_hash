...
from merge_duplicates_3 import run_merge, MergeRunConfig
...
def run_job(
    ...
    *,
    ...
    try:
        ...
        print_ts(f"Job '{job.name}' finished in {elapsed:.1f} seconds (ok={ok}, errors={stats.get('errors', 0)})")
...
def main() -> int:
    ...
    print_ts(f"All selected jobs finished in {total_elapsed:.1f} seconds (failures={any_failure})")
